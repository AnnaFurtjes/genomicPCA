---
title: "Principal component analysis (PCA) of Genome-Wide Association Study (GWAS) summary statistics"
output:
  html_document:
    theme: yeti
    includes:
      after_body: footer.html 
---

$$\\[0.01in]$$

Here we present a method we call **Genomic PCA** to derive GWAS summary statistics for a genetic principal component of multiple GWAS phenotypes derived from samples of unknown degrees of overlap using univariate summary statistics for the individual phenotypes. This procedure simply relies on publicly available GWAS summary statistics as input data. We make use of the [GenomicSEM software](https://github.com/GenomicSEM/GenomicSEM/wiki) and build on the work on genome-wide multivariate meta-analysis by [Baselmans et al. (2019)](https://pubmed.ncbi.nlm.nih.gov/30643256/). 


Authors: 

* Anna E. Fürtjes
* Elliot M. Tucker-Drob
* Javier de la Fuente
* Stuart J. Ritchie


***

Available code: 


  1) Click <span style="text-decoration: underline; color: #ff878d">[here](25082021_geneticPCA_explanation.html)</span> for a reproducible step-by-step demonstration of genomic PCA.


  2) Click <span style="text-decoration: underline; color: #ff878d">[here](Parallel_analysis.html)</span> for a reproducible demonstration of parallel analysis for genomic correlation matrices.

$$\\[0.1in]$$

<center>

![](C:/Users/k1894405/Documents/GitHub/genomicPCA/methods_visualisation.jpg){width=70%}

$$\\[0.1in]$$
</center>


