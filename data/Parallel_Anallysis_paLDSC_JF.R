...
# Date: 05-11-2020
#
# Filename: paLDSC_JFC.R
#
# Purpose: Defining a function to perform Parallel Analysis (PA) on LDSC genetic correlation matrices. 
# The method compares the eigenvalues generated from the PCA eigen decomposition (or the factorization) of the LDSC genetic 
# correlation matrix to the eigenvalues of a Monte-Carlo simulated null correlation matrix with random noise drawn from 
# the multivariate LDSC sampling distribution V. The suggested number of components (or factors if fa = T) to be extracted
# corresponds with the last component (or factor) with a larger eigenvalue than the same component (or factor) in the null
# correlation matrix.
...
paLDSC <- function(S_Stand = S_Stand, V_Stand = V_Stand, r = NULL, p = NULL, save.pdf = F, diag = F, fa = F,
  ...
  for (i in 1:r) {
    ...
    cat("Running PCA parallel analysis. Replication number: ",i,"\n")
  }
  ...
  #Create data frame from simulated PCA PA data
  ...
  #Create vector for observed minus PCA PA simulated data and # of factors
...
if (diag == T) { 
  ...
  for (i in 1:r) {
    ...
    cat("Running PCA diagonalized parallel analysis. Replication number: ",i,"\n")
  }
  
  #---- Find the p percentil for diagonalized PCA PA----#
  Paralleld_values=rowQuantiles(as.matrix(EIGd),probs=p)
  #Create data frame from diagonalized PCA PA simulated data
  ...
  #Create vector for observed minus diagonalized PCA PA simulated data and suggested number of components to be extracted
  ...
  if (fa == T) {
    ...
    if (diag == T){ 
      ...
      #Create vector for observed minus diagonalized PCA PA simulated data and # of factors
  ...
  pPA = ggplot(eigendatPA, aes(x=num, y=eigenvalue, shape=type)) +
    ...
    apatheme
  #---- Scree plot Observed minus PCA PA ----#
  ...
  pPAd = ggplot(eigendatPAd, aes(x=num, y=eigenvalue, shape=type)) +
    geom_line()+
    geom_point(size=3)+
    scale_y_continuous(name='Diagonalized PCA Eigenvalue')+
    scale_x_continuous(name='Component Number', breaks=min(1:k):max(1:k))+
    scale_shape_manual(values=c(2,17)) +
  ...
  pobsPAd = ggplot(obsPAd, aes(x=num, y=eigenvalue, shape=type)) +
    geom_line()+
    geom_point(size=3)+
    scale_y_continuous(name='Difference in diagonalized PCA Eigenvalues')+
    scale_x_continuous(name='Component Number', breaks=min(1:k):max(1:k))+
    scale_shape_manual(values=c(17)) +
  ...
  if (isTRUE(save.pdf)){
    figurePCA <- ggarrange(pPA, pobsPA, 
                          labels = c(NA,NA),
                          ncol = 1, nrow = 2)
    ggexport(figurePCA, filename = "PCA_PA_LDSC.pdf",res = 300, verbose = F)
    if (diag == T){
    figurePCAd <- ggarrange(pPAd, pobsPAd, 
                           labels = c(NA,NA),
                           ncol = 1, nrow = 2)
    ggexport(figurePCAd, filename = "PCA_Diagonalized_PA_LDSC.pdf",res = 300, verbose = F)
    }
    if (fa == T){
...
paLDSC(S_Stand = LDSCoutput$S_Stand, V_Stand = LDSCoutput$V_Stand, r = 100, p = .95, diag = F,
       fa = T, fm = "minres", nfactors = 1, save.pdf = T)
